...
fm_backend_paseo_create_task() { # <id> <source-clone> <brief> <harness> <model> <effort> <mode> <home-tag> [env key=value...]
  local id=$1 source=$2 brief=$3 harness=$4 model=${5:-} effort=${6:-} mode=${7:-} home_tag=${8:-}
  shift 8
...
fm_backend_paseo_capture() { # <agent-id> <lines>
  ...
  fm_backend_paseo_tool_check || return 1
  # Status is the watcher's lifecycle signal; logs are only the timeline peek.
  paseo inspect "$id" --json | jq -r '"[Paseo status: " + (.Status // .status // "unknown") + "]"' 2>/dev/null || return 1
...
fm_backend_paseo_kill() { # <agent-id> <workspace-id>
  ...
  fm_backend_paseo_tool_check || return 1
  paseo archive "$agent" >/dev/null || return 1
...
fm_backend_paseo_list_live() {
  local tag=${1:-} raw
  fm_backend_paseo_tool_check || return 1
  raw=$(paseo ls -a -g --json) || return 0
  printf '%s\n' "$raw" | jq -r --arg tag "$tag" '.[]? | select((.labels // {} | .["fm-home"] == $tag) or ($tag == "")) | "\(.id)\tfm-\(.labels["fm-task"] // .name)"' 2>/dev/null
}

fm_backend_paseo_current_path() {
  fm_backend_paseo_tool_check || return 1
  paseo inspect "$1" --json | jq -r '.Cwd // .cwd // empty'
}
