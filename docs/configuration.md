...
For Paseo, allowlisted values are written to a task-scoped mode-0600 file at `state/<task-id>.paseo-env` and passed only through Paseo's native `--env-file` option.
